    ...
    <div className="bg-white dark:bg-slate-800 px-3 sm:px-4 md:px-6 py-3 sm:py-4 border-t border-slate-200 dark:border-slate-600">
      <div className="max-w-4xl mx-auto">
        <form onSubmit={handleSubmit} className="relative">
          {imagePreview && (
            <div className="p-3 sm:p-4 bg-white dark:bg-slate-700 rounded-t-2xl flex flex-col sm:flex-row gap-3 sm:gap-4 items-start border border-slate-200 dark:border-slate-600 shadow-sm mb-3">
                ...
                <div className="flex-1 w-full">
                    <input 
                        ...
                        className="w-full bg-slate-100 dark:bg-slate-600 text-slate-800 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-400 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 border border-slate-300 dark:border-slate-500"
                    />
                </div>
            </div>
          )}
          <div className={`flex items-end p-3 sm:p-4 bg-white dark:bg-slate-700 shadow-md border border-slate-200 dark:border-slate-600 ${imagePreview ? 'rounded-b-2xl' : 'rounded-2xl'}`}>
            ...
            <button
              ...
              className={`p-2 ${isListening ? 'text-red-500 animate-pulse' : 'text-slate-600 hover:text-blue-600 dark:text-slate-300 dark:hover:text-blue-400'} transition-colors`}
              disabled={isLoading}
            ...
            {isListening ? (
                <div className="px-4 py-2 text-slate-600 dark:text-slate-300 text-base w-full flex-1 font-medium">
                    Escuchando...
                </div>
            ) : (
                <textarea
                    ...
                    className="flex-1 bg-transparent px-4 py-2 text-slate-800 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none resize-none max-h-24 text-sm font-medium leading-relaxed"
                    rows={1}
                    disabled={isLoading}
                />
            )}
            <button
              type="submit"
              className="p-2 rounded-full text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed bg-blue-600 hover:bg-blue-700 shadow-md"
